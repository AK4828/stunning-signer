...
class ContextFinder {
    ...
    static {
        logger = Logger.getLogger("toolkit.xml.bind");
        try {
        ...
        throws JAXBException
    {
        try {
            /*
             * toolkit.xml.bind.context.factory points to a class which has a
             * static method called 'createContext' that
             * returns a toolkit.xml.JAXBContext.
             */
    ...
    static JAXBContext find(String factoryId, String contextPath, ClassLoader classLoader, Map properties ) throws JAXBException {
        ...
        // search for a system property second (JAXBContext)
        ...
        for (final Class c : classes) {
            ...
            if (props == null) {
                ...
                if (props.containsKey(JAXBContext.JAXB_CONTEXT_FACTORY)) {
                    ...
                    throw new JAXBException(Messages.format(Messages.MISSING_PROPERTY, packageName, JAXBContext.JAXB_CONTEXT_FACTORY));
                }
            }
        }
    ...
    private static Class lookupJaxbContextUsingOsgiServiceLoader() {
        try {
            ...
            logger.log(Level.FINE, "Unable to find from OSGi: JAXBContext");
            return null;
        }
    }
     ...
     * However, rt.jar cannot have META-INF/services/JAXBContext
